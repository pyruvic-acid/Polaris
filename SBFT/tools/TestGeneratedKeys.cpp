...
//  if(participatingSigners < threshold) {
//      std::cout << "Number of signers=" << participatingSigners <<" is less than threshold=" << threshold<< std::endl;
//      return true;
//  }
  std::cout << "Number of signers=" << participatingSigners <<" and threshold=" << threshold<< std::endl;
...
static bool testThresholdKeys(const std::vector<bftEngine::ReplicaConfig>&
  ...
  std::cout << " --- Testing optimistic fast path commit ---.\n";
  signers.clear();
  verifiers.clear();
  for (uint16_t i = 0; i < numReplicas; ++i) {
