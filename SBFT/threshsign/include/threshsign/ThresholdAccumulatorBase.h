...
#include "bls/relic/BlsNumTypes.h"
...
public:
	...
	virtual int getNumValidShares() const {
        ...
        virtual std::vector<std::string> getValidShares() {
            std::vector<std::string> shares(validShares.size());
            for(ShareID id = validSharesBits.first(); validSharesBits.isEnd(id) == false; id = validSharesBits.next(id)) {
                size_t idx = static_cast<size_t>(id);
                shares[idx] = ((BLS::Relic::G1T)validShares[idx]).toString();
            }
            return shares;
	}
        virtual VectorOfShares getValidSharesBits() {
            return validSharesBits;
        };
