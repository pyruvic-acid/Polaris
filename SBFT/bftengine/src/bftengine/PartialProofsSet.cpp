...
namespace bftEngine {
    namespace impl {
        ...
        void PartialProofsSet::addSelfMsgAndPPDigest(PartialCommitProofMsg* m, Digest40& digest) {
            ...
            // name-removed: NOte: set expected digest also veify pending sharesverifyPendingShares
            thresholdAccumulator(m->commitPath())->setExpectedDigest(reinterpret_cast<unsigned char *> (digest.content()), sizeof(digest));
            selfPartialCommitProof = m;

            addImp(m, m->commitPath());
        }
        
        Digest40 PartialProofsSet::getExpectedDigest() {
        ...
        std::vector<std::string> PartialProofsSet::getValidShares() {
            CommitPath cPath = selfPartialCommitProof->commitPath();
            if (cPath == CommitPath::OPTIMISTIC_FAST) {
                return thresholdAccumulator(cPath)->getValidShares();
            } else {
                return thresholdAccumulator(cPath)->getValidShares();
            }
        }
        
        VectorOfShares PartialProofsSet::getValidSharesBits(){
            CommitPath cPath = selfPartialCommitProof->commitPath();
            IThresholdAccumulator* acc = thresholdAccumulator(cPath);
            
            return acc->getValidSharesBits();
        ...
        bool PartialProofsSet::addMsg(FullCommitProofMsg* m) {
            ...
            bool succ = thresholdVerifier(myPCP->commitPath())->verify((const char*) &expectedDigest, sizeof (Digest40),
        ...
        public:
            ...
            AsynchProofCreationJob(InternalReplicaApi* myReplica, IThresholdVerifier* verifier, IThresholdAccumulator* acc, Digest40& expectedDigest, SeqNum seqNumber, ViewNum viewNumber) {
                this->me = myReplica;
                this->acc = acc;
            ...
            virtual void execute() {
                ...
                // name-removed: verifies each signatuare using verification keys, and aggregateShares into one single signature-
                acc->getFullSignedData(bufferForSigComputations, sigLength);

                bool succ = verifier->verify((char*) &expectedDigest, sizeof (Digest40), bufferForSigComputations, (uint16_t) sigLength);
                ...
                if (!succ) {
                    ...
                    // name-removed: bufferForSigComputations is commitProofSig 
                    char *result = new char[sigLength * 2];
                    char *ptr = bufferForSigComputations;
                    for(int i = 0; i < sigLength; ++i){
                        snprintf(result + i*2, 2, "%02hhx", *ptr);
                        ++ptr;
                    }
                    LOG_INFO_F(GL, "------>Create FullProof for seqNumber %" PRId64 " sig=%s sigLen=%d", seqNumber, result, sigLength);
                    delete[] result;
