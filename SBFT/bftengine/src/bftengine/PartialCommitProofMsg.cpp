...
namespace bftEngine
{
	namespace impl
	{
		...
		PartialCommitProofMsg::PartialCommitProofMsg(ReplicaId senderId, ViewNum v, SeqNum s, CommitPath commitPath, Digest40& digest, IThresholdSigner* thresholdSigner)
			: MessageBase(senderId, MsgCode::PartialCommitProof, sizeof(PartialCommitProofMsgHeader) + thresholdSigner->requiredLengthForSignedData())
		{
			uint16_t thresholSignatureLength = (uint16_t)thresholdSigner->requiredLengthForSignedData();
