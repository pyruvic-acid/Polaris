...
namespace bftEngine
{
	namespace impl
	{
		...
		bool SeqNumInfo::addMsg(PrePrepareMsg* m)
		{
			...
			// set expected
			Digest40 tmpDigest;
			Digest40::calcCombination(m->digestOfRequests(), m->viewNumber(), m->seqNumber(), tmpDigest);
		...
		bool SeqNumInfo::addSelfCommitPartialMsgAndDigest(CommitPartialMsg *m, Digest40& commitDigest)
		{
			Assert(replica->getReplicasInfo().myId() == m->senderId());
			Assert(!forcedCompleted);
			...
			// set expected
			Digest40 tmpDigest;
			Digest40::calcCombination(commitDigest, m->viewNumber(), m->seqNumber(), tmpDigest);
		...
		Digest40 SeqNumInfo::getExpectedDigest() {
			return commitMsgsCollector->getExpectedDigest();
		}
