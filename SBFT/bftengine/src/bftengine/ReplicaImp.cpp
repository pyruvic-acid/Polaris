...
namespace bftEngine {
    namespace impl {
        ...
        std::unordered_map<uint16_t, PtrToMetaMsgHandler> ReplicaImp::createMapOfMetaMsgHandlers() {
            ...
            // name-removed: Change ClientRequestMsg to bftengine::duanqn::SkipCycleClientReqMsg
            r[MsgCode::Request] = &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<bftEngine::duanqn::SkipCycleClientReqMsg>;
        ...
        void ReplicaImp::onMessage(ClientRequestMsg *m) {
            // name-removed: receives message from client
            ...
            if (seqNumberOfLastReply < reqSeqNum) {
                if (isCurrentPrimary()) {
                    if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum) && (requestsQueueOfPrimary.size() < 700)) // TODO(GG): use config/parameter
                    {
                        ...
                        LOG_INFO_F(GL, "ClientRequestMsg is ignored becuase: request is old, OR primary is current working on a request from the same client, OR queue contains too many requests (size=%d)", requestsQueueOfPrimary.size());
                    }
        ...
        void ReplicaImp::onMessage(bftEngine::duanqn::SkipCycleClientReqMsg *m) {
            // name-removed: Need to implement
            // name-removed: receives message from client
            metric_received_client_requests_.Get().Inc();
            const NodeIdType senderId = m->senderId();
            const NodeIdType clientId = m->clientProxyId();
            ...
            LOG_INFO_F(GL, "Node %d received SkipCycleClientReqMsg (clientId=%d reqSeqNum=%" PRIu64 ", readOnly=%d) from Node %d", myReplicaId, clientId, reqSeqNum, readOnly ? 1 : 0, senderId);

            if (stateTransfer->isCollectingState()) {
                LOG_INFO_F(GL, "SkipCycleClientReqMsg is ignored becuase this replica is collecting missing state from the other replicas");
                // name-removed: This object is constructed from MessageBase
                delete (MessageBase *)m;
                return;
            }
            ...
            if (invalidClient || sentFromReplicaToNonPrimary) // TODO(GG): more conditions (make sure that a request of client A cannot be generated by client B!=A)
            {
                LOG_INFO_F(GL, "SkipCycleClientReqMsg is invalid");
                onReportAboutInvalidMessage(m);
                // name-removed: This object is constructed from MessageBase
                delete (MessageBase *)m;
                return;
            ...
            if (readOnly) {
                LOG_INFO_F(GL, "duanqn: Readonly requests are not supported!");
                // name-removed: This object is constructed from MessageBase
                delete (MessageBase *)m;
                throw 5;
                return;
            ...
            if (!currentViewIsActive()) {
                LOG_INFO_F(GL, "SkipCycleClientReqMsg is ignored becuase current view is inactive");
                // name-removed: This object is constructed from MessageBase
                delete (MessageBase *)m;
                return;
            }
            ...
            if (seqNumberOfLastReply < reqSeqNum) {
                if (isCurrentPrimary()) {
                    if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum) && (requestsQueueOfPrimary.size() < 700)) // TODO(GG): use config/parameter
                    {
                        ...
                        LOG_INFO_F(GL, "SkipCycleClientReqMsg is ignored because: request is old, OR primary is current working on a request from the same client, OR queue contains too many requests (size=%d, max=700)", requestsQueueOfPrimary.size());
                    }
                } else // not the current primary
                {
                    if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum)) {
                        ...
                        LOG_INFO_F(GL, "Sending SkipCycleClientReqMsg to current primary");
                    } else {
                        LOG_INFO_F(GL, "SkipCycleClientReqMsg is ignored becuase request is old or replica has another pending request from the same client");
                    }
                }
            } else if (seqNumberOfLastReply == reqSeqNum) {
                LOG_INFO_F(GL, "SkipCycleClientReqMsg has already been executed - retransmit reply to client");
                ...
                LOG_INFO_F(GL, "SkipCycleClientReqMsg is ignored becuase request is old");
            }
        ...
        void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic) {
            ...
            // name-removed: NOTE requests are batched (but for some reason the batch size is 1) 
            // Even when then batch size is set to 4, there is only one request
            //            LOG_INFO_F(GL, "+++++++++++++ Entering tryToSendPrePrepareMsg() Try to Send PrepareMessage");
            ...
            bftEngine::duanqn::SkipCycleClientReqMsg* first = requestsQueueOfPrimary.front();
            ...
            minBatchSize = 1; //force one batch at a time?
            ...
            // name-removed: preprepare message multiple requests? why?
            bftEngine::duanqn::SkipCycleClientReqMsg* nextRequest = requestsQueueOfPrimary.front();
            int maxSkipCycleSuggestion = bftEngine::duanqn::NOSKIP;
            //            LOG_INFO_F(GL, "Entering ---- +++++++++++++ requestsQueueSzize=%d, BatchSize=%d", (int)requestsQueueOfPrimary.size(), (int) minBatchSize);
            while (nextRequest != nullptr && nextRequest->size() <= pp->remainingSizeForRequests()) {
                if (clientsManager->noPendingAndRequestCanBecomePending(nextRequest->clientProxyId(), nextRequest->requestSeqNum())) {
                    ...
                    int skipCycleHint = nextRequest->skipCycleHint();
                    if(skipCycleHint != bftEngine::duanqn::NOSKIP){
                        // name-removed: TODO - Check its proof!
                        
                        if(skipCycleHint > maxSkipCycleSuggestion){
                            maxSkipCycleSuggestion = skipCycleHint;
                        }
                    }
                ...
                // name-removed: This object is constructed from MessageBase
                delete (MessageBase *)nextRequest;
                requestsQueueOfPrimary.pop();
                nextRequest = (requestsQueueOfPrimary.size() > 0 ? requestsQueueOfPrimary.front() : nullptr);
#ifndef BATCHING
                break; //name-removed: when no batching, ensure only one request is added
#endif
            }

            static uint32_t s_lastCycleNumberAssigned = 0u;

            // First cycle should get cycle number 1, if there is no SkipCycle
            uint32_t currentCycleNumberAssigned = (maxSkipCycleSuggestion != bftEngine::duanqn::NOSKIP && (uint32_t)maxSkipCycleSuggestion > s_lastCycleNumberAssigned) ? maxSkipCycleSuggestion : s_lastCycleNumberAssigned + 1;
            pp->setLastCycleNumber(s_lastCycleNumberAssigned);
            pp->setCurrentCycleNumber(currentCycleNumberAssigned);

            pp->finishAddingRequests();

            // Update last cycle number
            s_lastCycleNumberAssigned = currentCycleNumberAssigned;
        ...
        void ReplicaImp::onMessage(PrePrepareMsg* msg) {
            ...
            // name-removed: Should walk through the PrePrepareMsg, inspect all cycleNumberHint fields in client requests, and check if the primary provided the current & last cycle numbers correctly
            // name-removed: But we just trust the primary for now
        ...
        void ReplicaImp::sendPartialProof(SeqNumInfo& seqNumInfo) {
            ...
            Digest40& ppDigest = pp->digestOfRequests();
            ...
            if (!partialProofs.hasFullProof()) {
                ...
                if (part == nullptr) {
                    ...
                    Digest40 tmpDigest;
                    Digest40::calcCombination(ppDigest, curView, seqNum, tmpDigest);

                    part = new PartialCommitProofMsg(myReplicaId, curView, seqNum, commitPath, tmpDigest, commitSigner);
                    //name-removed: tmpDigest == expected Digest
        ...
        void ReplicaImp::sendPreparePartial(SeqNumInfo& seqNumInfo) {
            ...
            if (seqNumInfo.getSelfPreparePartialMsg() == nullptr && seqNumInfo.hasPrePrepareMsg() && !seqNumInfo.isPrepared()) {
                ...
                // name-removed: what to use for slow path?
        ...
        void ReplicaImp::sendCommitPartial(const SeqNum s) {
            ...
            Digest40 d;
            Digest40::digestOfDigest(pp->digestOfRequests(), d);
...

        {
            ...
            LOG_INFO_F(GL, "name-removed: Server ID: %d to %d; Client ID: %d to %d", 0, numOfReplicas - 1, numOfReplicas, numOfReplicas + numOfClientProxies - 1);
        ...
        void ReplicaImp::executeRequestsInPrePrepareMsg(PrePrepareMsg* ppMsg) {
            ...
            // name-removed: each preppare message consists of multiple client requests. 
            // the commit proof (threshold signature) is created against a batch of requests
            // although it is committed one request at a time. The threshold sig is thus meaningless unless it is committed for all requests
            // 
            RequestsIterator reqIter(ppMsg);
            char* requestBody = nullptr;
            uint16_t numOfReq = ppMsg->numberOfRequests();
            LOG_INFO_F(GL, "Replica:%d  entering requests: #reqests:%d", (int) myReplicaId, numOfReq);

            SeqNumInfo& seqNumInfo = mainLog->get(lastExecutedSeqNum + 1);
            PartialProofsSet& pps = seqNumInfo.partialProofs();

            // FastPath has a FullCommitProofMsg
            FullCommitProofMsg* fcp = pps.getFullProof();
            // SlowPath has a CommitFullMsg
            CommitFullMsg* commitFull = seqNumInfo.getValidCommitFullMsg();
            ...
            SeqNum seqNum = ppMsg->seqNumber();
            ViewNum viewNum = ppMsg->viewNumber();

            if (fcp != nullptr) {
                // fast path result
                LOG_INFO_F(GL, "The Msg's consensus is achieved through fast path.");

                thresholdSignature = fcp->thresholSignature();
                thresholdSignatureLength = fcp->thresholSignatureLength();

                expectedDigest = pps.getExpectedDigest();

            } else if (commitFull != nullptr) {
                // slow path result
                LOG_INFO_F(GL, "The Msg's consensus is achieved through slow path.");

                thresholdSignature = commitFull->signatureBody();
                thresholdSignatureLength = commitFull->signatureLen();

                expectedDigest = seqNumInfo.getExpectedDigest();
            } else {
                LOG_FATAL_F(GL, "The Msg has no FullCommitProofMsg (for fastPath) or CommitFullMsg (for slowPath), fatal error.");
            }

            if (numOfReq == 0) {


                int error = userRequestsHandler->executeEmpty(seqNum, viewNum, expectedDigest, thresholdSignature, thresholdSignatureLength);

                Assert(error == 0);
                
            } else {
                Assert(lastExecutedSeqNum + 1 == seqNum);

                std::vector<std::pair <const char*, size_t>> commands;
                ...
                while (reqIter.getAndGoToNext(requestBody)) {
                    ...
                    Assert(req.isReadOnly() == false); // polaris does not have read only requests to sbft

                    std::pair <const char*, size_t> cmdContent(req.requestBuf(), req.requestLength());
                    commands.push_back(cmdContent);
                }

                uint32_t actualReplyLength = 0;

                int error = userRequestsHandler->executeMulti(
                        commands,
                        lastExecutedSeqNum + 1, viewNum, expectedDigest, thresholdSignature, thresholdSignatureLength,
