...
namespace bftEngine
{
	namespace impl
	{
		...
		void ViewChangeMsg::getMsgDigest(Digest& outDigest) const
		{
			...
			DigestUtil::compute(body(), bodySize, (char*)outDigest.content(), sizeof(outDigest));
		}

		void ViewChangeMsg::addElement(const ReplicasInfo& repInfo, SeqNum  seqNum, const Digest40& prePrepreDigest, ViewNum   originView,
