...
namespace bftEngine
{
	namespace impl
	{
		...
		struct FastElem
		{
			...
			Digest40& prePrepreDigest() const { return e->prePrepreDigest; }
		};
		...
		struct SlowElemCompare
		{
			bool operator()(const SlowElem& lhs, const SlowElem& rhs) const
			{
				...
				else
				{
					...
					const Digest40& leftPPDigest = lhs.prePrepreDigest();
					const Digest40& rightPPDigest = rhs.prePrepreDigest();

					return ( (memcmp(&leftPPDigest, &rightPPDigest, sizeof(Digest40))) < 0);
				}
			}
		};
		...
		ViewChangeSafetyLogic::ViewChangeSafetyLogic(const uint16_t n, const uint16_t f, const uint16_t c, IThresholdVerifier* const preparedCertificateVerifier, const Digest40& digestOfNull) :
			N(n),
			F(f),
			C(c),
		...
		bool ViewChangeSafetyLogic::computeRestrictionsForSeqNum(SeqNum s, vector<ViewChangeMsg::ElementsIterator*>& VCIterators, const SeqNum upperBound, Digest40& outRestrictedDigest) const
		{
			...
			for (SlowElem slow : slowPathCertificates)
			{
				...
				Digest40 d;
				Digest40::calcCombination(slow.prePrepreDigest(), slow.certificateView(), slow.seqNum(), d);

				bool valid = preparedCertVerifier->verify(d.content(), sizeof(d), slow.certificateSig(), slow.certificateSigLength());
