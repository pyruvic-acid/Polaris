...
namespace bftEngine
{
	namespace impl
	{
		...
		protected:
			...
			bftEngine::duanqn::SkipCycleClientReqMsg* pendingRequest = nullptr;
		...
		{
                    //name-removed: Gets message from a replic - may be execute-ack nsg
			...
			Assert(replyMsg->type() == REPLY_MSG_TYPE);
 			...
 			LOG_INFO_F(GL, "Client %d received ClientReplyMsg with s seqNum=%"
			PRIu64
		...
		{			
			...
			// name-removed: Change to bftEngine::duanqn::SkipCycleClientReqMsg
			char const * const realRequest = request + 2 * sizeof(unsigned int);
			uint32_t realLength = lengthOfRequest - 2 * sizeof(unsigned int);
			unsigned int proofOffset = *(unsigned int *)(request);
			unsigned int cycleNumber = *(unsigned int *)(request + sizeof(unsigned int));

			bftEngine::duanqn::SkipCycleClientReqMsg* reqMsg = new bftEngine::duanqn::SkipCycleClientReqMsg(_clientId, isReadOnly, reqSeqNum, realLength, realRequest, int(proofOffset), int(cycleNumber));
			...
			while (true)
			{
				...
				if (((uint64_t)absDifference(timeOfLastTransmission, currTime))/1000 > (firstRetransmit ? 100 : limitOfExpectedOperationTime.upperLimit()))
				{
					onRetransmission();
				}
			}
			...
			if (requestCommitted)
			{
				...
				{
                                    //name-removed: copys the output to the reply buffer
