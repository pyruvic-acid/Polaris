...
class AsyncTcpConnection :
 ...
 public:
  void connect(string ip, uint16_t port, bool destIsReplica) {
    ...
    LOG_INFO(_logger, "connecting from: " << _selfId
              << " ,to: " << _destId
              << ", ip: " << ip
              << ", port: " << port
...
class PlainTCPCommunication::PlainTcpImpl {
 private:
  ...
  void on_hello_message(NodeNum id, ASYNC_CONN_PTR conn) {
    ...
    lock_guard<recursive_mutex> lock(_connectionsGuard);
    // if id already exists, erase before inserting.
    auto curConn = _connections.find(id);
    if (curConn != _connections.end()) {
      _connections.erase(id);
      curConn->second->close();
    }
  ...
  PlainTcpImpl(NodeNum selfNodeId,
    ...
    // All replicas and clients start listening
    ...
    for (auto it = nodes.begin(); it != nodes.end(); it++) {
      ...
      if (isSubstitute && _selfId <= _maxServerId) {
        // When a replica is restarting, connect to all other replicas and clients
        toConnect = (it->first != _selfId) ? true : false;
      } else {
        ...
        toConnect = it->first < _selfId && it->first <= maxServerId;
      }

      if (toConnect) {
        LOG_INFO(_logger, "Connecting from Node: " << _selfId << " to Node: " << to_string(it->first));
        auto conn =
            AsyncTcpConnection::
            create(&_service,
                   std::bind(
