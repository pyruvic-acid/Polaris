...
IDBClient* ReplicaImp::getDB() const
{
	return this->getBcDbAdapter()->getDb();
...
Status ReplicaImp::addBlockInternal(const SetOfKeyValuePairs& updates, BlockId& outBlockId)
{
	StateTransferDigest digestOfPrev;
        // name-removed: remove the digest; not required
        memset((char*)&digestOfPrev, 0, sizeof(digestOfPrev));
        // name-removed Comment out : not needed
//	if (lastBlock == 0)
//	{
//		memset((char*)&digestOfPrev, 0, sizeof(digestOfPrev));
//	}
...
//	}
	...
	{
            std::cout << "ERROR: blocksize: " <<  blockRaw.size << " max block:" << maxBlockSize << std::endl;
		return Status::IllegalBlockSize("Block is too big");
	}
//        std::cout << "INFO: blocksize: " <<  blockRaw.size << " max block:" << maxBlockSize << std::endl;
	...
	lastBlock++;
//        std::cout << "INFO: updating key:  "<< updatesInNewBlock.size() << std::endl;
...
void ReplicaImp::insertBlockInternal(BlockId blockId, Slice block)
{
	...
	if (!s.ok())
	{
        ...
        // if block is found return else add 
...
bool ReplicaImp::executeCommand(uint16_t clientId,
	...
	if (readOnly)
	{
		size_t replySize = outActualReplySize;
...
bool ReplicaImp::executeCommand(uint16_t clientId,
	...
	{
		size_t replySize = outActualReplySize;
		bool ret = m_cmdHandler->executeCommand(cmdContent, *this, *this, maxReplySize, outReply, replySize,
                        sequenceNum,viewNum, digest, thresholdSignature, thresholdSignatureLen); // TODO(GG): ret vals
		outActualReplySize = replySize;
		return ret;
	}
}

bool ReplicaImp::executeMultiCommands(const std::vector<std::pair <const char*, size_t>>& commands,
	uint64_t sequenceNum, 
	uint64_t viewNum, 
	...
	std::vector<Slice> commandsSlice;
	for (const auto& command : commands) {
		Slice cmdContent(command.first, command.second);
		commandsSlice.push_back(cmdContent);
	}

	size_t replySize = outActualReplySize;
	bool ret = m_cmdHandler->executeMultiCommands(commandsSlice, *this, *this, maxReplySize, outReply, replySize,
					sequenceNum, viewNum, digest, thresholdSignature, thresholdSignatureLen); // TODO(GG): ret vals
	outActualReplySize = replySize;
	return ret;
...
bool ReplicaImp::executeEmptyCommand(uint64_t sequenceNum,
			uint64_t viewNum, 
			bftEngine::impl::Digest40 digest, 
			const char* thresholdSignature, 
			uint16_t thresholdSignatureLen) {

	bool ret = m_cmdHandler->executeEmptyCommand(*this, *this, sequenceNum, viewNum, digest, thresholdSignature, thresholdSignatureLen); 
	return ret;
}
...
Slice ReplicaImp::createBlockFromUpdates(const SetOfKeyValuePairs& updates, SetOfKeyValuePairs& outUpdatesInNewBlock, StateTransferDigest digestOfPrev)
{
	...
	int64_t blockBodySize = 0;
	int64_t numOfElemens = 0;
//        std::cout << "DEBUG: ReplicaImp::createBlockFromUpdates() update size:" << updates.size() << std::endl;
	for (auto it = updates.begin(); it != updates.end(); ++it)
	{
		const KeyValuePair& kvPair = KeyValuePair(it->first, it->second);
		numOfElemens++;
//                std::cout << "DEBUG: ReplicaImp::createBlockFromUpdates() firstsize :" << kvPair.first.size << " secondsize:" << kvPair.second.size << std::endl;
		blockBodySize += (int64_t)(kvPair.first.size + kvPair.second.size);
	}
        
        
        
	const int64_t headerSize = sizeof(blockHeader) - sizeof(blockEntry) + sizeof(blockEntry)*(numOfElemens);
	const int64_t blockSize = headerSize + blockBodySize;
//        std::cout << "DEBUG: ReplicaImp::createBlockFromUpdates() blockBodySize :" << blockBodySize << " numElems:" << numOfElemens << " blocksize:"  << blockSize<< std::endl;
//        assert(updates.size() == numOfElemens);
	try {
		...
		header->numberOfElements = numOfElemens;
                // name-removed: Not required; TODO: comment later
//		header->prevBlockDigest = digestOfPrev;
		...
		for (auto it = updates.begin(); it != updates.end(); ++it)
		{
			...
			header->entries[idx].keySize = (int64_t)kvPair.first.size;
			...
			currentOffset += (int64_t)kvPair.first.size;

			// value
			header->entries[idx].valOffset = currentOffset;
			header->entries[idx].valSize = (int64_t)kvPair.second.size;
			...
			currentOffset += (int64_t)kvPair.second.size;
		...
		return Slice(blockBuffer, blockSize);
                ...
                std::cout << " Failed to alloc size: " << blockSize << std::endl;
                exit(1);
...
SetOfKeyValuePairs ReplicaImp::fetchBlockData(Slice block)
{
	...
	if (block.size > 0)
	{
		...
		char* begin = (char *)malloc(block.size);
        memcpy(begin, block.data, block.size);
		blockHeader* header = (blockHeader*)begin;
        std::cout << "DEBUG: Fetching block Data: #elems" << header->numberOfElements << std::endl;
		for (int16_t i = 0; i < header->numberOfElements; i++)
		{
			const char* key = begin + header->entries[i].keyOffset;
			const int64_t keyLen = header->entries[i].keySize;
			const char* val = begin + header->entries[i].valOffset;
			const int64_t valLen = header->entries[i].valSize;

			PolarisKey* compositeKey = (PolarisKey*) key;
			compositeKey->countInBlock = i;

			Slice keySlice((const char*) compositeKey, keyLen);
...
int RequestsHandlerImp::executeMulti(const std::vector<std::pair <const char*, size_t>>& commands,
		uint64_t sequenceNum, 
        uint64_t viewNum, 
        bftEngine::impl::Digest40 digest, 
        const char* thresholdSignature, uint16_t thresholdSignatureLen, 
	...
	ReplicaImp* r = m_Executor;
	bool ret = r->executeMultiCommands(commands, 
            sequenceNum, viewNum, digest, thresholdSignature, thresholdSignatureLen,
        	maxReplySize, outReply, outActualReplySize);
	return ret?0:1;
}

int RequestsHandlerImp::executeEmpty(uint64_t sequenceNum,
			uint64_t viewNum, 
			bftEngine::impl::Digest40 digest, 
			const char* thresholdSignature, 
			uint16_t thresholdSignatureLen) {
	...
	bool ret = r->executeEmptyCommand(sequenceNum, viewNum, digest, thresholdSignature, thresholdSignatureLen);
	return ret?0:1;
