...
namespace SimpleKVBC {
	...
	protected:
		...
		bool executeMultiCommands(const std::vector<std::pair <const char*, size_t>>& commands,
			uint64_t sequenceNum, 
			uint64_t viewNum, 
		...
		bool executeEmptyCommand(uint64_t sequenceNum,
			uint64_t viewNum, 
			bftEngine::impl::Digest40 digest, 
			const char* thresholdSignature, 
...
//                        virtual IDBClient* getDB() { return m_db;}
//                        virtual TKVStore::const_iterator cend() { return m_iter->cend();};
		};
	...
	public:
		...
		int executeMulti(const std::vector<std::pair <const char*, size_t>>& commands,
			uint64_t sequenceNum, 
			uint64_t viewNum, 
			bftEngine::impl::Digest40 digest, 
			const char* thresholdSignature, uint16_t thresholdSignatureLen, 
		...
		int executeEmpty(uint64_t sequenceNum,
			uint64_t viewNum, 
			bftEngine::impl::Digest40 digest, 
			const char* thresholdSignature, 
			uint16_t thresholdSignatureLen) override;
	};
