...
{
//    std::cout << "INFO: inserting value to kv" << std::endl;
    ...
    map[key] = value;
//    std::cout << "INFO: inserted value successfully to kv" << std::endl;
    return Status::OK();
}
...
KeyValuePair InMemoryDBClientIterator::getCurrent()
{
    if (m_current == m_parentClient->getMap().end())
    {
         std::cout<< "END getCurrent()" << std::endl;
        return KeyValuePair();
    }
    std::cout<< "YEND getCurrent()" << std::endl;
    return KeyValuePair(m_current->first, m_current->second);
}
